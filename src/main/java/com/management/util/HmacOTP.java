...
public class HmacOTP {
    ...
    public static String generateSecret(int length) {
        ...
        for (int i = 0; i < length; i++) {
            char charAt = chars.charAt(random.nextInt(chars.length()));
            sb.append(charAt);
        }
        return sb.toString();
    }
     ...
     * @param counter represents counter
     * @return -1 if not a match.  A positive number means successful validation.
     * This positive number is also the new value of the counter
     */
     ...
     * @throws java.security.GeneralSecurityException when exceptional condition happens
     */
    public String generateOTP(String key, String counter, int returnDigits, String crypto) {
        ...
        hash = hmacSha1(crypto, keyByte, msg);
     ...
     * @throws java.security.NoSuchAlgorithmException when  exceptional condition happens
     * @throws java.security.InvalidKeyException      when key is invalid
     */
    private byte[] hmacSha1(String crypto, byte[] keyBytes, byte[] text) throws GeneralTechnicalException {
        byte[] value;
    ...
    private byte[] hexStr2Bytes(String hex) {
        ...
        System.arraycopy(byteArray, 1, ret, 0, ret.length);
        return ret;
    }
